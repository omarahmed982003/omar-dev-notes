...
كلمة **Server** تصف دورًا، لا شكل جهاز بعينه. قد تعني:

- **جهازًا فعليًا أو Virtual Machine** يوفّر CPU وRAM وشبكة وتخزينًا.
- **برنامجًا** يستمع على عنوان IP ومنفذ ويجيب وفق Protocol، مثل Nginx أو PostgreSQL.
- **دورًا في تفاعل محدد:** البرنامج الذي يطلب خدمة هو Client، والذي يقدمها Server. وقد يكون التطبيق نفسه Client لخادم قاعدة البيانات.

يبدأ Server Process عادة بفتح Socket وربطه بعنوان ومنفذ ثم الاستماع. عندما يصل اتصال، يقرأ الطلب ويتحقق منه وينفذ العمل ويرسل استجابة. تشغيل `localhost:8000` لا يعني وجود جهاز مستقل؛ يعني أن Process على جهازك يستمع محليًا على المنفذ `8000`.

يمكن لجهاز واحد تشغيل Web Server وApplication Server وDatabase في التطوير، بينما تفصلها بيئة الإنتاج لأسباب التوسع والعزل والأمان. وعبارة «السيرفر وقع» غير دقيقة حتى نحدد: هل الجهاز متوقف، أم العملية، أم الشبكة، أم خدمة تابعة؟
